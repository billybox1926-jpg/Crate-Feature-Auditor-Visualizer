...
fn re_root_remote_metadata(
    ...
    let workspace_member = metadata
        ...
        .cloned()
        .ok_or(Error::Remote(
            "remote metadata did not include the probe workspace member".into(),
        ))?;
    let probe_node = metadata
        ...
        .ok_or(Error::Remote(
            "remote metadata did not include the probe resolve node".into(),
        ))?;
    let target_id = probe_node
        ...
        .ok_or_else(|| {
            Error::Remote(format!(
                "crate `{crate_name}` was not resolved from crates.io"
            ))
        })?;
